...
def display_hand(hand):
    ...
    for letter in hand.keys():
        for j in range(hand[letter]):
            hand_string += f"{letter} "      # print all on the same line
...
def is_valid_word(word, hand, word_list):
    ...
    possible_words = []
    asterisk_index = word_lower.find("*")
    if(asterisk_index != -1):
        for vowel in VOWELS:
            new_word = word_lower[:asterisk_index] + vowel + word_lower[asterisk_index+1:]
            possible_words.append(new_word)
    else:
        possible_words = word_lower 
        

    for words in word_list:
        if(words.lower() in possible_words):
            return True
    ...
    while(len(hand) != 0):
        ...
        # Otherwise (the input is not two exclamation points):
            ...
            if(is_valid_word(word, hand, word_list)):
                ...
                word_points = get_word_score(word, len(hand))
                total_score += word_points
                print(f'"{word}" earned {word_points} points. Total: {total_score} points')
    ...
    * Returns the total score for the series of hands
    ...
    while(num_of_hands != 0):
        ...
        while(substitute != "yes" and substitute != "no"):
            substitute = input("Would you like to substitue a letter? (yes/no) : ")
        if(substitute == "yes"):
        ...
        if(replay_hand == "yes"):
            total_score -= hand_score
            total_score += play_hand(original_hand, word_list)
